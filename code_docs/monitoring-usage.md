<!-- Source: https://code.claude.com/docs/zh-CN/monitoring-usage -->

Title: 监控 - Claude Code Docs

URL Source: https://code.claude.com/docs/zh-CN/monitoring-usage

Markdown Content:
Claude Code 支持 OpenTelemetry (OTel) 指标和事件，用于监控和可观测性。所有指标都是通过 OpenTelemetry 的标准指标协议导出的时间序列数据，事件通过 OpenTelemetry 的日志/事件协议导出。用户有责任确保其指标和日志后端配置正确，并且聚合粒度满足其监控要求。
...
管理员可以通过托管设置文件为所有用户配置 OpenTelemetry 设置。这允许在整个组织中集中控制遥测设置。有关设置如何应用的更多信息，请参阅 [设置优先级](https://code.claude.com/docs/zh-CN/settings#settings-precedence)。托管设置文件位于：
...
这些变量有助于控制指标的基数，这会影响指标后端中的存储要求和查询性能。较低的基数通常意味着更好的性能和更低的存储成本，但数据分析的粒度较低。
...
**标头仅在启动时获取，不在运行时获取。** 这是由于 OpenTelemetry 导出器架构的限制。对于需要频繁令牌刷新的场景，使用 OpenTelemetry Collector 作为代理，它可以刷新自己的标头。

### 多团队组织支持

具有多个团队或部门的组织可以使用 `OTEL_RESOURCE_ATTRIBUTES` 环境变量添加自定义属性来区分不同的组：
...
*   所有 [标准属性](https://code.claude.com/docs/zh-CN/monitoring-usage#standard-attributes)
...
通过 Claude Code 创建拉取请求时递增。**属性**：

*   所有 [标准属性](https://code.claude.com/docs/zh-CN/monitoring-usage#standard-attributes)

#### 提交计数器

通过 Claude Code 创建 git 提交时递增。**属性**：
...
当用户接受或拒绝 Edit、Write 或 NotebookEdit 工具使用时递增。**属性**：
...
跟踪实际花费在积极使用 Claude Code 上的时间（不是空闲时间）。此指标在用户交互期间递增，例如输入提示或接收响应。**属性**：
...
当用户提交提示时记录。**事件名称**：`claude_code.user_prompt`**属性**：

*   所有 [标准属性](https://code.claude.com/docs/zh-CN/monitoring-usage#standard-attributes)
*   `event.name`：`"user_prompt"`
*   `event.timestamp`：ISO 8601 时间戳
*   `prompt_length`：提示的长度
*   `prompt`：提示内容（默认情况下已编辑，使用 `OTEL_LOG_USER_PROMPTS=1` 启用）

#### 工具结果事件

当工具完成执行时记录。**事件名称**：`claude_code.tool_result`**属性**：
...
为每个对 Claude 的 API 请求记录。**事件名称**：`claude_code.api_request`**属性**：
...
当对 Claude 的 API 请求失败时记录。**事件名称**：`claude_code.api_error`**属性**：
...
当做出工具权限决策（接受/拒绝）时记录。**事件名称**：`claude_code.tool_decision`**属性**：

*   所有 [标准属性](https://code.claude.com/docs/zh-CN/monitoring-usage#standard-attributes)
*   `event.name`：`"tool_decision"`
*   `event.timestamp`：ISO 8601 时间戳
*   `tool_name`：工具的名称（例如 “Read”、“Edit”、“Write”、“NotebookEdit” 等）
*   `decision`：`"accept"` 或 `"reject"`
*   `source`：决策来源 - `"config"`、`"user_permanent"`、`"user_temporary"`、`"user_abort"` 或 `"user_reject"`
...
事件数据提供了对 Claude Code 交互的详细见解：**工具使用模式**：分析工具结果事件以识别：
...
有关测量 Claude Code 投资回报率的综合指南，包括遥测设置、成本分析、生产力指标和自动报告，请参阅 [Claude Code ROI 测量指南](https://github.com/anthropics/claude-code-monitoring-guide)。此存储库提供现成的 Docker Compose 配置、Prometheus 和 OpenTelemetry 设置，以及用于生成与 Linear 等工具集成的生产力报告的模板。
...
*   用户提示内容默认情况下已编辑 - 仅记录提示长度。要启用用户提示日志记录，请设置 `OTEL_LOG_USER_PROMPTS=1`
