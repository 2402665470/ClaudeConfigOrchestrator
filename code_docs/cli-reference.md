...
Title: CLI 参考 - Claude Code Docs

URL Source: https://code.claude.com/docs/zh-CN/cli-reference
...
| `claude mcp` | 配置模型上下文协议 (MCP) 服务器 | 请参阅 [Claude Code MCP 文档](https://code.claude.com/docs/zh-CN/mcp)。 |
...
| `--agents` | 通过 JSON 动态定义自定义 [子代理](https://code.claude.com/docs/zh-CN/sub-agents)（参见下面的格式） | `claude --agents '{"reviewer":{"description":"Reviews code","prompt":"You are a code reviewer"}}'` |
| `--allowedTools` | 应允许的工具列表，无需提示用户获得权限，除了 [settings.json 文件](https://code.claude.com/docs/zh-CN/settings) | `"Bash(git log:*)" "Bash(git diff:*)" "Read"` |
| `--disallowedTools` | 应禁止的工具列表，无需提示用户获得权限，除了 [settings.json 文件](https://code.claude.com/docs/zh-CN/settings) | `"Bash(git log:*)" "Bash(git diff:*)" "Edit"` |
| `--print`, `-p` | 打印响应而不进入交互模式（有关程序化使用详情，请参阅 [SDK 文档](https://docs.claude.com/en/docs/agent-sdk)） | `claude -p "query"` |
...
| `--permission-mode` | 以指定的 [权限模式](https://code.claude.com/docs/zh-CN/iam#permission-modes) 开始 | `claude --permission-mode plan` |
...
有关创建和使用子代理的更多详情，请参阅 [子代理文档](https://code.claude.com/docs/zh-CN/sub-agents)。
...
有关打印模式 (`-p`) 的详细信息，包括输出格式、流、详细日志记录和程序化使用，请参阅 [SDK 文档](https://docs.claude.com/en/docs/agent-sdk)。

另请参阅
----

*   [交互模式](https://code.claude.com/docs/zh-CN/interactive-mode) - 快捷键、输入模式和交互功能
*   [斜杠命令](https://code.claude.com/docs/zh-CN/slash-commands) - 交互会话命令
